...
export function musicRepeatCount(seconds: number): number {
  return seconds < 45 ? 3 : seconds < 90 ? 2 : 1;
}
...
const watchClock = (tick: () => void) => {
  const timer = window.setInterval(tick, 200);
  return () => window.clearInterval(timer);
};

/** Playback and repeat counts follow the audio clock, even after tab suspension. */
export function createMusicPlayer({ getOutput, load, fallback, onTrack = () => {}, watch = watchClock, random = Math.random }: Dependencies) {
  ...
  let voice: {
    ...
    started: number; elapsed: number; ends: number;
  } | undefined;
  ...
  async function start(mode: TrackMode, output: Output, index: number, elapsed: number) {
    ...
    for (let attempt = 0; attempt < candidates.length; attempt++) {
      ...
      try {
        ...
        const total = buffer.duration * musicRepeatCount(buffer.duration);
        if (!(total > 0) || !Number.isFinite(total)) throw new Error("Invalid music duration");
        // A suspended tab may resume after its scheduled medley boundary.
        const played = attempt === 0 && elapsed < total - .8 ? elapsed : 0;
        ...
        source.start(0, played % buffer.duration);
        fallback(null);
        fadeOut(false);
        positions[mode] = { index: trackIndex, elapsed: played };
        voice = { source, bus, mode, index: trackIndex, started: audio.currentTime,
          elapsed: played, ends: audio.currentTime + total - played };
        loading = false;
        nextIndex = pickNextMusicIndex(trackIndex, playlist.length, random);
        onTrack(track);
        cancelWatch?.();
        cancelWatch = watch(() => {
          if (!voice || loading || requested !== mode || audio.state !== "running") return;
          if (audio.currentTime >= voice.ends - 1.2) void next();
        });
        // Start downloading the next song before its fade. Failure is retried
        // at the boundary; the current loop continues until another is ready.
        void load(playlist[nextIndex], audio).catch(() => {});
        return;
      } catch {
        if (ticket !== generation) return;
      }
    ...
    loading = false;
    cancelWatch?.(); cancelWatch = undefined;
    fadeOut(false);
    fallback(mode);
  ...
  async function setMode(mode: MusicMode): Promise<void> {
    if (mode === requested) return;
    const output = mode ? getOutput() : undefined;
    if (mode && !output) return;
    // Save the previous playlist's place so breaks/round ends don't always
    // restart song one. Hold its audio until the replacement is decoded.
    ...
    await start(mode, output, position.index < 0 ? pickNextMusicIndex(-1, MUSIC_PLAYLISTS[mode].length, random) : position.index, position.elapsed);
  }
  return { setMode, next };
}

// Keep only the current/next few decoded tracks, rather than every full PCM song.
...
export function setMusicMode(mode: MusicMode): void { void player.setMode(mode); }
// The dev-only QA fixture can exercise a boundary without waiting two minutes.
